...
# ==================== STUDENT DASHBOARD (with Attendance) ====================
@app.route("/student")
def student_dashboard():
    ...
    """
    if attendance:
        html += "<table border='1' cellpadding='10' style='width:100%; border-collapse: collapse;'><tr><th>Date</th><th>Module</th><th>Status</th><th>Hours</th><th>Notes</th></tr>"
        for a in attendance:
            html += f"<tr><td>{a['date']}</td><td>{a['module_name']}</td><td>{a['status']}</td><td>{a['hours_attended']}</td><td>{a['notes'] or ''}</td></tr>"
        html += "</table>"
    else:
...
def contractor_dashboard():
    ...
    html = f"<div style='max-width:900px; margin:40px auto; font-family:Arial;'><h2>Your Apprentices</h2><p>Contractor: {email}</p>"
    if students:
        for s in students:
...
# ==================== INSTRUCTOR DASHBOARD (Attendance Tracking) ====================
...
def instructor_dashboard():
    ...
    <div style="max-width: 650px; margin: 40px auto; font-family: Arial;">
        ...
        <form method="POST">
            ...
            <label>Date:</label><br>
            <input type="date" name="date" value="{{ today }}" style="width:100%; padding:10px; margin-bottom:15px;"><br>
