...
def render_page(title, content):
    ...
    <head>
        ...
        <style>
            /* Castellar font specifically for the logo text */
            .font-castellar {{ font-family: 'Castellar', 'Times New Roman', serif; letter-spacing: 0.05em; }}
            
            /* Professional, slightly larger base font size to match standard websites */
        ...
        <div class="bg-white border border-gray-200 border-t-0 shadow-lg rounded-b-2xl p-10 md:p-12">
            <form method="POST" class="space-y-10">
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <input type="text" name="phone" class="input-std md:w-1/2">
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-gray-50 p-8 rounded-xl border border-gray-200">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div>
                            <label class="block text-base font-bold text-gray-700 mb-2">Contractor Name</label>
                            <input type="text" name="contractor_name" class="input-std bg-white">
                        </div>
                        <div>
                            <label class="block text-base font-bold text-gray-700 mb-2">Contractor Email</label>
                            <input type="email" name="contractor_email" class="input-std bg-white">
                        </div>
                    </div>
                </div>

                <div class="pt-6">
                    <button type="submit" class="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-2xl py-5 rounded-xl shadow-md transition">
...
def register_instructor():
    ...
    <div class="bg-white border border-gray-200 shadow-sm rounded-2xl p-10 md:p-12 max-w-md mx-auto mt-12">
        ...
        <form method="POST" class="space-y-6">
            <div>
                <label class="block text-base font-bold text-gray-700 mb-2">Choose Username</label>
                <input type="text" name="username" required class="input-std">
            </div>
            <div>
                <label class="block text-base font-bold text-gray-700 mb-2">Choose Password</label>
                <input type="password" name="password" required class="input-std">
            </div>
            <div class="pt-4">
                <button type="submit" class="btn-green w-full py-4 text-xl">Create Account</button>
            </div>
        </form>
        <div class="mt-8 text-center">
            <a href="/login" class="text-gray-500 hover:text-emerald-700 font-medium hover:underline text-lg">Back to Login</a>
        </div>
    </div>
    """
...
def contractor_dashboard():
    ...
    for s in students:
        ...
        if grades:
            ...
            """
            for g in grades: 
                html += f"<tr class='border-b hover:bg-gray-50 transition'><td class='p-4'>{g['module_name']}</td><td class='p-4 font-bold text-gray-900'>{g['grade']}</td><td class='p-4 font-medium'>{g['hours_attended']}</td></tr>"
            html += "</table></div></div>"
        else:
...
def instructor_dashboard():
    ...
    if request.method == "POST":
        if "delete_id" in request.form:
            conn.execute("DELETE FROM grades_hours WHERE id = ?", (request.form['delete_id'],))
        elif "edit_id" in request.form:
            conn.execute("UPDATE grades_hours SET module_name=?, grade=?, hours_attended=? WHERE id=?", 
                         (request.form['edit_module_name'], request.form['edit_grade'], request.form['edit_hours_attended'], request.form['edit_id']))
        else:
    ...
    for s in students:
        ...
        <div class="bg-white border border-gray-200 rounded-2xl p-8 mb-10 shadow-sm">
            ...
            <form method='POST' class="flex flex-col md:flex-row gap-6 bg-emerald-50 p-6 rounded-xl mb-8 items-end border border-emerald-100">
                ...
                <div class="w-full md:flex-grow"><label class="block text-sm font-bold text-emerald-800 uppercase mb-2">Module / Class</label><input name='mod' class="input-std"></div>
                <div class="w-full md:w-32"><label class="block text-sm font-bold text-emerald-800 uppercase mb-2">Grade</label><input name='grd' class="input-std"></div>
                <div class="w-full md:w-32"><label class="block text-sm font-bold text-emerald-800 uppercase mb-2">Hours</label><input type="number" name='hrs' class="input-std"></div>
