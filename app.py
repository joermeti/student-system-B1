...
def init_db():
    ...
    # NEW TABLE: Tracks payments returned from Stripe
...
def enroll():
    if request.method == "POST":
        ...
        try:
            ...
            # Determine initial payment amount based on selection
            # Stripe requires amounts in cents (e.g., 50000 = $500.00)
            payment_amount_cents = 50000  # Default deposit for payment plans
            if "Paid in Full" in payment_plan:
                payment_amount_cents = 240000 # Example: $2,400 Paid in Full
            ...
            checkout_session = stripe.checkout.Session.create(
                ...
                mode='payment',
                client_reference_id=str(student_id), # We attach the student ID so we can verify it later
...
def payment_success():
    ...
    try:
        # Retrieve the session from Stripe to verify it actually happened
        checkout_session = stripe.checkout.Session.retrieve(session_id)
        student_id = checkout_session.client_reference_id
        amount_paid = f"${checkout_session.amount_total / 100:.2f}"
        
        conn = get_db()
        # Prevent logging the same payment twice if the user refreshes the page
    ...
    <div class="bg-white border border-gray-200 shadow-sm rounded-2xl p-14 max-w-2xl mx-auto text-center mt-12">
        <div class="text-emerald-600 mb-6"><svg class="w-20 h-20 mx-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg></div>
...
if __name__ == "__main__": app.run(debug=True)app.secret_key = "rmeti-secret-2026"
