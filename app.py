...
stripe.api_key = "sk_test_51Tk4WNRTNA1TU9kTBDbPvGXbHJfNYPR35sZl5wXgLKvmqMkUoKd4WurJqQHmqpLnU5zgpMLzs9Cftp0OABfy0TdJ00C5yw3qr0"
...
def enroll():
    if request.method == "POST":
        ...
        try:
            ...
            total_cost = 2400 # Default to Main Program
...
def instructor_settings():
    ...
    if request.method == "POST":
        if "update_profile" in request.form:
            ...
            conn.execute("DELETE FROM instructors WHERE username=?", (instructor_name,))
            conn.commit()
            session.clear()
            flash("Instructor account deleted permanently.")
            return redirect(url_for("home"))
            
    inst = conn.execute("SELECT * FROM instructors WHERE username = ?", (instructor_name,)).fetchone()
    conn.close()
...
def student_dashboard():
    ...
    if payments:
        ...
        for p in payments: html += f"<li class='flex justify-between items-center text-lg'><span class='text-gray-600'>{p['date_recorded']}</span> <span class='font-bold text-emerald-700 bg-emerald-50 px-3 py-1 rounded'>{p['amount_paid']} - {p['status']}</span></li>"
        html += "</ul>"
    else:
    ...
    if grades:
        ...
        """
        for g in grades:
            html += f"<tr class='border-b hover:bg-gray-50 transition'><td class='py-3 px-4'>{g['module_name']}</td><td class='py-3 px-4 text-emerald-700 font-semibold'>{g['recorded_by']}</td><td class='py-3 px-4 font-bold text-gray-900'>{g['grade']}</td><td class='py-3 px-4 font-medium'>{g['hours_attended']}</td><td class='py-3 px-4 text-gray-500 text-sm'>{g['recorded_date']}</td></tr>"
        html += "</table></div>"
    else:
...
def contractor_dashboard():
    ...
    """
    if not students: html += "<div class='bg-white p-8 rounded-2xl border border-gray-200 shadow-sm'><p class='text-lg text-gray-600'>No apprentices found for your email address.</p></div>"
    ...
    for s in students:
        ...
        if grades:
            ...
            for g in grades: html += f"<tr class='border-b hover:bg-gray-50 transition'><td class='py-3 px-4'>{g['module_name']}</td><td class='py-3 px-4 text-emerald-700 font-semibold'>{g['recorded_by']}</td><td class='py-3 px-4 font-bold text-gray-900'>{g['grade']}</td><td class='py-3 px-4 font-medium'>{g['hours_attended']}</td><td class='py-3 px-4 text-gray-500 text-sm'>{g['recorded_date']}</td></tr>"
            html += "</table></div></div>"
        else: html += "<p class='text-gray-500 italic text-base'>No grades recorded yet.</p></div>"
    conn.close()
        ...
        <div class="bg-white border border-gray-200 rounded-2xl p-8 mb-10 shadow-sm">
            ...
            <div class="flex flex-col md:flex-row justify-between items-start mb-6 border-b border-gray-100 pb-4">
                <div>
                    ...
                    <p class="text-lg text-emerald-700 font-medium">{s['program']} <span class="text-gray-400 mx-2">|</span> <span class="text-gray-600 text-base">{s['email']}</span></p>
                </div>
