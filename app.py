...
stripe.api_key = "sk_test_51QBIjDBBkd8fbCjdlTPr2FdrlEdG2p1s2awRpizLOWRaF5FLgQVZBDEgxKtj7ODEVEllmQlsqP2NzK77ytFx7G5y00S1WiYUVh"
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if "Paid in Full" in payment_plan:
                ...
                try:
                    # Extracts the '4' from '4-Month Payment Plan'
                    months = int(payment_plan.split('-')[0]) 
                    # Breaks it down into equal fractions
                    monthly_payment = total_cost / months 
                    payment_amount_cents = int(monthly_payment * 100)
                except:
                    payment_amount_cents = 50000 # Safety fallback
            
            checkout_session = stripe.checkout.Session.create(
                payment_method_types=['card'],
                customer_email=email, # THIS SENDS THE AUTOMATIC RECEIPT TO THE STUDENT
                line_items=[{
                    'price_data': {
                        'currency': 'usd',
