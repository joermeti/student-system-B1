...
def init_db():
    ...
    # Instructors table (for individual passwords)
    c.execute("""
...
def enroll():
    if request.method == "POST":
        ...
        if not full_name or not email or not program:
            flash("Full Name, Email, and Program are required.", "error")
        ...
        try:
            conn.execute("""
                ...
                (full_name, email, phone, program, payment_plan, contractor_name, contractor_email, enrollment_date)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                full_name, email, phone, program, payment_plan,
    ...
    <div class="max-w-2xl mx-auto mt-10 bg-white p-10 rounded-3xl shadow-xl border">
        ...
        <form method="POST" class="space-y-5">
            ...
            <button type="submit" class="w-full bg-emerald-600 hover:bg-emerald-700 text-white py-4 rounded-2xl font-medium mt-4">
                Enroll Student
            </button>
        </form>
    </div>
    ...
    <div class="max-w-md mx-auto mt-20 bg-white p-10 rounded-3xl shadow-xl border">
        ...
        <form method="POST" class="space-y-6">
            <div>
                ...
                <select name="role" id="role" class="w-full border border-gray-300 rounded-2xl px-5 py-4 text-lg" onchange="togglePassword()">
            ...
            <div>
                <label class="block text-sm mb-2 text-gray-600 font-medium">Email or Username</label>
                <input type="text" name="identifier" required class="w-full border border-gray-300 rounded-2xl px-5 py-4 text-lg">
            </div>

            <div id="passwordField" style="display: none;">
                <label class="block text-sm mb-2 text-gray-600 font-medium">Password</label>
                <input type="password" name="password" class="w-full border border-gray-300 rounded-2xl px-5 py-4 text-lg">
            </div>

            <button type="submit" class="w-full bg-emerald-600 hover:bg-emerald-700 text-white py-4 rounded-2xl text-lg font-medium mt-2">
                Login
            </button>
        </form>

        <div class="mt-6 text-center">
            <a href="/register_instructor" class="text-emerald-600 hover:underline text-sm">Register as Instructor</a>
        </div>
    </div>
...
def contractor_dashboard():
    ...
    students = get_students_by_contractor(email)
