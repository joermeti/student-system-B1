...
def contractor_dashboard():
    ...
    students = conn.execute("""
        ...
        ORDER BY full_name
    """, (contractor_email,)).fetchall()
    conn.close()

    html = f"<h2>Your Apprentices</h2><p>Contractor: {contractor_email}</p>"
    
    if students:
        html += "<ul>"
        for s in students:
            html += f"<li>{s['full_name']} — {s['semester']} ({s['payment_plan']})</li>"
        html += "</ul>"
    else:
...
def instructor_dashboard():
    ...
    <div style="max-width:1100px;margin:30px auto;font-family:Arial;">
        ...
        <p>Add grades and hours. History shown below each student.</p>
    """
    ...
    for s in students:
        ...
        if grades:
            ...
            for g in grades:
                html += f"<tr><td>{g['module_name'] or '-'}</td><td>{g['grade'] or '-'}</td><td>{g['hours_attended'] or '-'}</td><td>{g['recorded_date']}</td><td>{g['recorded_by']}</td></tr>"
            html += "</table>"
        else:
...
@app.route("/success")
def success():
    return "<h2>Success!</h2><a href='/enroll'>Enroll more</a> | <a href='/contractor'>Contractor View</a>"
