...
def home():
    ...
    <p><a href='/students'>View All Enrolled Students</a></p>
    """
...
def success():
    ...
    <p><a href='/enroll'>Enroll another student</a></p>
...
def students():
    ...
    html = "<h2>Enrolled Students</h2>"
    if not all_students:
        html += "<p>No students enrolled yet.</p>"
    else:
        html += "<table border='1' cellpadding='8'>"
        html += "<tr><th>Name</th><th>Email</th><th>Phone</th><th>Semester</th><th>Payment Plan</th><th>Date</th></tr>"
        for s in all_students:
            html += f"<tr><td>{s['full_name']}</td><td>{s['email']}</td><td>{s['phone']}</td><td>{s['semester']}</td><td>{s['payment_plan']}</td><td>{s['enrollment_date']}</td></tr>"
        html += "</table>"
