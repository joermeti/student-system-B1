...
def login():
    if request.method == "POST":
        ...
        if role == "instructor":
            ...
            # Student or Contractor login (no password required)
            if role == "student":
                conn = get_db()
    ...
    <div class="max-w-md mx-auto mt-20 bg-white p-10 rounded-3xl shadow-xl border">
        ...
        <form method="POST" class="space-y-6">
            ...
            <div>
                <label class="block text-sm mb-2 text-gray-600 font-medium">Username / Email</label>
...
def register_instructor():
    if request.method == "POST":
        ...
        try:
            ...
            conn.commit()
            flash("Instructor account created successfully. You can now log in.")
            return redirect(url_for("login"))
