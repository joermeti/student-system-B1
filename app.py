...
def home():
    ...
    <div class="max-w-5xl mx-auto px-6 py-12">
        ...
        <div class="bg-white shadow-xl rounded-3xl p-10 border">
            <h2 class="text-3xl font-semibold mb-4">Welcome to the RMETI Portal</h2>
            <p class="text-gray-600 max-w-md">Manage student enrollments, track progress, and record grades & hours.</p>
        </div>
    </div>
    """
...
def login():
    ...
    return """
    <div class="max-w-md mx-auto mt-16 bg-white p-8 rounded-3xl shadow-xl border">
        <h2 class="text-2xl font-semibold mb-6">Login</h2>
        <form method="POST">
            <div class="mb-4">
                <label class="block text-sm mb-1 text-gray-600">I am a</label>
                <select name="role" class="w-full border rounded-2xl px-4 py-3">
            ...
            <div class="mb-6">
                <label class="block text-sm mb-1 text-gray-600">Email or Name</label>
                <input type="text" name="identifier" required class="w-full border rounded-2xl px-4 py-3">
            </div>
            <button type="submit" class="w-full bg-emerald-600 hover:bg-emerald-700 text-white py-3 rounded-2xl font-medium">Login</button>
        </form>
    </div>
    """
...
def student_dashboard():
    ...
    <div class="max-w-4xl mx-auto mt-10">
        ...
        <p class="mt-4">Welcome! Your information and progress will appear here.</p>
        <a href="/logout" class="text-red-600 mt-6 inline-block">Logout</a>
    </div>
    """
...
def contractor_dashboard():
    ...
    conn = get_db()
    students = conn.execute("SELECT * FROM students WHERE contractor_email = ?", (email,)).fetchall()
    conn.close()

    html = f"""
    <div class="max-w-5xl mx-auto mt-10">
        <h2 class="text-3xl font-semibold mb-6">Your Apprentices</h2>
        <p class="text-gray-600 mb-4">Showing students for: <strong>{email}</strong></p>
    """
    if students:
        ...
        for s in students:
            ...
            <div class="bg-white border rounded-2xl p-6">
                <h3 class="font-semibold text-lg">{s['full_name']}</h3>
                <p class="text-sm text-gray-500">{s['semester']} • {s['payment_plan']}</p>
            </div>
            """
        html += "</div>"
    else:
        html += "<p>No students found for this contractor.</p>"
    html += "<br><a href='/logout' class='text-red-600'>Logout</a></div>"
    return html
...
def instructor_dashboard():
    ...
    <div class="max-w-6xl mx-auto mt-8 px-6">
        <h2 class="text-3xl font-semibold mb-8">Instructor Dashboard</h2>
    """
    ...
    for s in students:
        ...
        <div class="bg-white border rounded-3xl p-8 mb-8 shadow-sm">
            <div class="flex justify-between items-start mb-6">
                <div>
                    <h3 class="text-2xl font-semibold">{s['full_name']}</h3>
                    <p class="text-gray-500">{s['semester']}</p>
                </div>
            </div>
        ...
        if grades:
            ...
            html += "<p class='text-gray-500 text-sm'>No records yet.</p>"
...
def enroll():
    ...
    <div class="max-w-2xl mx-auto mt-10 bg-white p-10 rounded-3xl shadow-xl border">
        ...
        <form method="POST" class="space-y-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    <label class="block text-sm mb-1 text-gray-600">Semester</label>
                    <input type="text" name="semester" value="Fall 2026" class="w-full border rounded-2xl px-4 py-3">
                </div>
                <div>
